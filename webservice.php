...
//echo "<pre>";print_r($input_params);die;
		if(!isset($firstName) || empty($userName) || empty($email) || !isset($birthDate) ||  empty($lastName) || empty($password))
		{
...
$img =  "http://" . $_SERVER['SERVER_NAME'].'/service/admin/images/'.$inactive_state['image'];
			$pages[] = array('product_id'=> $inactive_state['product_id'],
		...
		if(empty($pages))
		{
			...
			header('Content-type: application/json');
			echo json_encode(array('status'=>1,'ProductInfo'=>$pages,'message'=>'Get product details  successfully.'));
